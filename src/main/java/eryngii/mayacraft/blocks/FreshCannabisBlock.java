...
public class FreshCannabisBlock   extends Block implements IGrowable {
    ...
    public FreshCannabisBlock() {
    	super(Material.cactus);
        setCreativeTab(MayaCraftCore.tabsMaya);/*クリエイティブタブの選択*/
        setBlockName("blockFreshCannabis");/*システム名の設定*/
    ...
    @Override
    // ドロップするアイテム。未確認だが、とりあえずこんな感じで処理できるはず。乾燥後のブロックの仕様を考え中なので仮に粉を登録
    public Item getItemDropped(int metadata, Random random, int fortune) {
    	if(metadata==1){return MayaCraftCore.itemDryCannabisLeaf;}
    	else{
    		return  MayaCraftCore.itemCannabisLeaf;}
    ...
    public void registerBlockIcons(IIconRegister parIIconRegister)
    {
          ...
          // 全成長段階のテクスチャを登録
          iIcon[0] = parIIconRegister.registerIcon("mayacraft:freshcannabis");
          iIcon[1] = parIIconRegister.registerIcon("mayacraft:drycannabis");
